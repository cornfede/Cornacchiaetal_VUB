...
# Converting NewYork_nofilt.csv into spatial data using 'st_read' function of the sf package, the 'option'
...
options(digits = 9)
NewYork_nofilt <- st_read("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/NewYork_nofilt.csv", 
...
ggplot() + 
  geom_sf(data = NewYork_nofilt)

# I load the created shapefile of the neighborhood tabulation areas (NTAs) of NewYork

newyork_nta_shp <- st_read("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Shapefiles_NTA_Counties/NewYork_NTA.shp")


# Plotting vector data

ggplot() + 
  geom_sf(data = newyork_nta_shp)

# Looking up the Coordinate Reference System (WGS84 for ebird data and NAD83 for NYC nta and ct map)

st_crs(newyork_nta_shp)
st_crs(NewYork_nofilt)
...
nad83_newyork_nofilt <- st_transform(NewYork_nofilt, 2263)
st_crs(nad83_newyork_nofilt)

# Plotting together the different layers with the same CRS (NTAs and no filtered ebird data)

ggplot() + 
  geom_sf(data = newyork_nta_shp) + 
  geom_sf(data = nad83_newyork_nofilt)
...
spatial_join <- newyork_nta_shp %>% 
  st_join(nad83_newyork_nofilt)
...
S_NTA_NewYork <- summarise(species_by_nta, S = n_distinct(sci_name))
...
H_NTA_NewYork <- summarise(n_ind_per_species, H = diversity(n_ind, index = "shannon"))

# Exporting the S and H dataframes as .csv files

write.table(S_NTA_NewYork, 
            file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Species_Richness_and_Diversity_per_NTA_per_County/NewYork_NTA/S_NewYork.csv", 
            row.names = F, sep = ",")
write.table(H_NTA_NewYork, 
            file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Species_Richness_and_Diversity_per_NTA_per_County/NewYork_NTA/H_NewYork.csv", 
            row.names = F, sep = ",")

# Check if the produced .csv file is correct

options(digits = 9)
S_NewYork <- read.csv("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Species_Richness_and_Diversity_per_NTA_per_County/NewYork_NTA/S_NewYork.csv", header = TRUE, sep = ",")
H_NewYork <- read.csv("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Species_Richness_and_Diversity_per_NTA_per_County/NewYork_NTA/H_NewYork.csv", header = TRUE, sep = ",")
...
# I load the created shapefile of NewYork census tracts

newyork_ct_shp <- st_read("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Shapefiles_Census_Tracts_Counties/NewYork_CT.shp")

# Plotting vector data

ggplot() + 
  geom_sf(data = newyork_ct_shp)

# Looking up the Coordinate Reference System (WGS84 for ebird data and NAD83 for NYC ct map)

st_crs(newyork_ct_shp)
st_crs(NewYork_nofilt)
...
S_CT_NewYork <- summarise(species_by_ct, S = n_distinct(sci_name))

# Renaming column

colnames(S_CT_NewYork)[colnames(S_CT_NewYork) == "CTLabel"] <- "CTLABEL"

# Adding column named 'BORONAME' filled with 'Manhattan'

S_CT_NewYork$BORONAME <- "Manhattan"

# Now I want to combine the content of 'CTLABEL' and 'BORONAME' in one column named CT_BOR

S_CT_NewYork <- unite(S_CT_NewYork, CT_BOR, CTLABEL, BORONAME, sep = "-")
...
H_CT_NewYork <- summarise(n_ind_per_species, H = diversity(n_ind, index = "shannon"))

# Renaming column

colnames(H_CT_NewYork)[colnames(H_CT_NewYork) == "CTLabel"] <- "CTLABEL"

# Adding column named 'BORONAME' filled with 'Manhattan'

H_CT_NewYork$BORONAME <- "Manhattan"

# Now I want to combine the content of 'CTLABEL' and 'BORONAME' in one column named CT_BOR

H_CT_NewYork <- unite(H_CT_NewYork, CT_BOR, CTLABEL, BORONAME, sep = "-")

# Exporting the S and H dataframes as .csv file

write.table(S_CT_NewYork, file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Species_Richness_and_Diversity_per_Census_Tract_per_County/NewYork_ct/S_CT_NewYork.csv", row.names = F, sep = ",")
write.table(H_CT_NewYork, file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Species_Richness_and_Diversity_per_Census_Tract_per_County/NewYork_ct/H_CT_NewYork.csv", row.names = F, sep = ",")
