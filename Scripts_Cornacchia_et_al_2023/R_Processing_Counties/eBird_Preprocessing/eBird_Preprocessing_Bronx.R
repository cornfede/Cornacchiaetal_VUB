...
Bronx <- read.delim("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/eBird_Observations_Textfiles/Observations_Bronx_County.txt", quote="", stringsAsFactors=TRUE)
...
df_bronx <- Bronx %>% select(GLOBAL.UNIQUE.IDENTIFIER, COMMON.NAME, SCIENTIFIC.NAME, OBSERVATION.COUNT, COUNTY, LOCALITY, LOCALITY.TYPE, LATITUDE, LONGITUDE, OBSERVATION.DATE, PROTOCOL.TYPE, DURATION.MINUTES, EFFORT.DISTANCE.KM, NUMBER.OBSERVERS, ALL.SPECIES.REPORTED)

# Select detections only (removing all non-detections from the data)

det_only <- df_bronx[df_bronx$OBSERVATION.COUNT != "X", ]
...
# Turn all the NA into 0.03 (I choose to use 0.03 instead of 0 because for stationary observation protocol the maximum spatial range of movement to belong to this category is 30 meters or 0.03 km)
...
write.table(protocol_checklists, 
            file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/Bronx_nofilt.csv", 
            row.names = F, sep = ",")

# Check if the produced .csv file is correct

options(digits = 9)
Bronx_nofilt <- read.csv("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/Bronx_nofilt.csv", header = TRUE, sep = ",")
