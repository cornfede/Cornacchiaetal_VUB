...
Kings <- read.delim("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/eBird_Observations_Textfiles/Observations_Kings_County.txt", quote="", stringsAsFactors=TRUE)
...
df_kings <- Kings %>% select(GLOBAL.UNIQUE.IDENTIFIER, COMMON.NAME, SCIENTIFIC.NAME, OBSERVATION.COUNT, COUNTY, LOCALITY, LOCALITY.TYPE, LATITUDE, LONGITUDE, OBSERVATION.DATE, PROTOCOL.TYPE, DURATION.MINUTES, EFFORT.DISTANCE.KM, NUMBER.OBSERVERS, ALL.SPECIES.REPORTED)

# Select detections only (removing all non-detections from the data)

det_only <- df_kings[df_kings$OBSERVATION.COUNT != "X", ]
...
# Turn all the NA of the effort distance in 30 m (0.03 km)
...
write.table(protocol_checklists, 
            file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/Kings_nofilt.csv", 
            row.names = F, sep = ",")

# Check if the produced .csv file is correct

options(digits = 9)
Kings_nofilt <- read.csv("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/Kings_nofilt.csv", header = TRUE, sep = ",")
