...
Queens <- read.delim("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/eBird_Observations_Textfiles/Observations_Queens_County.txt", quote="", stringsAsFactors=TRUE)
...
df_queens <- Queens %>% select(GLOBAL.UNIQUE.IDENTIFIER, COMMON.NAME, SCIENTIFIC.NAME, OBSERVATION.COUNT, COUNTY, LOCALITY, LOCALITY.TYPE, LATITUDE, LONGITUDE, OBSERVATION.DATE, PROTOCOL.TYPE, DURATION.MINUTES, EFFORT.DISTANCE.KM, NUMBER.OBSERVERS, ALL.SPECIES.REPORTED)

# Select detections only (removing all non-detections from the data)

det_only <- df_queens[df_queens$OBSERVATION.COUNT != "X", ]
...
write.table(protocol_checklists, 
            file = "C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/Queens_nofilt.csv", 
            row.names = F, sep = ",")

# Check if the produced .csv file is correct

options(digits = 9)
Queens_nofilt <- read.csv("C:/Files/ESA/MSc Thesis Project/R_&_ArcGIS/NYC/R_Processing_Counties/Counties_csv/Queens_nofilt.csv", header = TRUE, sep = ",")
